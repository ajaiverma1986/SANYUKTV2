...
namespace SANYUKT.Provider.Payout
{
    public class RblPayoutProvider : BaseProvider
    {
        ...
        public async Task<SimpleResponse> PayoutTransaction(SinglePaymentRequest req, X509Certificate2 Certificatetext, ISANYUKTServiceUser serviceUser)
        {
            ...
            long benid = 0;
            if (req.BenficiaryId == null && req.BenficiaryId=="0")
            {
                if(req.Ben_Acct_No=="")
                {
                    response.SetError(ErrorCodes.BEN_DETAILS_MISS);
                    return response;
                }
            }
            else
            ...
            bp.Debit_Mobile = SANYUKTApplicationConfiguration.Instance.RblPayoutMobile;
            ...
            if (response1.StatusCode == HttpStatusCode.OK)
            {
                ...
                else if(resp.Status == "In process")
                {
                    request3.status = 3;
                }
